   ...
   function getAlternateAuthSystemsUserLogin($authtype = 0) {
      ...
      switch ($authtype) {
         ...
         case self::COOKIE:
            ...
            setcookie($cookie_name, '', time() - 3600, $cookie_path, null, isset($_SERVER["HTTPS"]), true);
   ...
   function login($login_name, $login_password, $noauto = false, $remember_me = false, $login_auth = '') {
      ...
      if ($this->auth_succeded && $CFG_GLPI['login_remember_time'] > 0 && $remember_me) {
         ...
         if ($token) {
            ...
            setcookie($cookie_name, $data, time() + $CFG_GLPI['login_remember_time'], $cookie_path, null, isset($_SERVER["HTTPS"]), true);
